...
//    self.title = [NSString stringWithFormat:format, (unsigned long)indexPaths.count];
    ...
    NSIndexPath * indexP=[indexPaths objectAtIndex:0];
    OLFacebookImage * photo=[self.photos objectAtIndex:indexP.row];
    
    
    NSString *urlStr = photo.fullURL.absoluteString;
    
    
        NSDictionary* userInfo = @{@"urlStr": urlStr};
    
    [[NSNotificationCenter defaultCenter] postNotificationName:@"callDoneManually"
                                                        object:self
                                                      userInfo:userInfo];
