//
//  EditThirdImageViewController.m
//  InterestMe
//
...
@interface EditThirdImageViewController ()
...
@implementation EditThirdImageViewController
...
BOOL SAVING_NEW_THIRD_IMAGE=NO;
-(void)useImage {
    //transfer image to parent...
    if (self.image && !SAVING_NEW_THIRD_IMAGE) {
        SAVING_NEW_THIRD_IMAGE=YES;
        ...
        [pictureFile saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
            if(!error)
            {
                ...
                [PFUser currentUser][@"thirdPicture"]=pictureFile;
                
                [[PFUser currentUser] saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
                    if(!error)
                    {
                        // Save the image, then pop the controller
                        SAVING_NEW_THIRD_IMAGE=NO;
